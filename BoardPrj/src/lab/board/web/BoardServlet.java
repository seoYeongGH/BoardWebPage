...
public class BoardServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("write_do".equals(action)) {
			...
			response.sendRedirect("/M3/Board.do?action=list");
			return;
			...
			if(dbpw.equals(password)) {
				...
				url = "/M3/Board.do?action=view&bbsno="+bbsno;
				response.sendRedirect(url);
				return;
			}else {
				request.setAttribute("message", "비밀번호가 다릅니다.<br><br>수정되지 않았습니다.");
				url = url+"/error/error.jsp";
			}
			...
			if(dbpw.equals(password)) {
				...
				request.setAttribute("message", "비밀번호가 다릅니다.<br><br>삭제되지 않았습니다.");
				url = url + "/error/error.jsp";
			}
		} else {
